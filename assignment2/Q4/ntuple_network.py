"""
N-Tuple Network for Battle Fib-2584.
Evaluates a single 4x4 board: high value = healthy/open board.
Used as: score = V(my_board) - V(opp_board) for action selection.

Tuple patterns (from proven 2584 experience):
  6-tuple A: (0,1,2,4,5,6)  -- top-left 2x3 block
  6-tuple B: (4,5,6,8,9,10) -- middle 2x3 block
  4-tuple A: (0,1,2,3)      -- top row
  4-tuple B: (4,5,6,7)      -- second row
Each with all 8 symmetries (4 rotations x 2 reflections).

Max tile code capped at 17 -> NUM_CODES = 18.
"""

import numpy as np

NUM_CODES = 18
MAX_CODE = NUM_CODES - 1  # 17
...
class NTupleNetwork:
    def __init__(self):
        ...
        # Precompute for fast single evaluate/update
        self._syms_6 = []
        self._syms_4 = []
    ...
    def evaluate(self, state_codes):
        """Evaluate a single board. state_codes: list of 16 ints (already clamped)."""
        c = state_codes
        NC = self._NC
        total = 0.0
    ...
    def update(self, state_codes, delta):
        """Update weights for a single board state."""
        c = state_codes
        NC = self._NC
        for lut, p0, p1, p2, p3, p4, p5 in self._syms_6:
    ...
    def evaluate_batch(self, states_codes):
        """Evaluate multiple board states. states_codes: numpy (N, 16) int array."""
    ...
    def load(self, filepath):
        ...
        # Rebuild references after loading
        self._syms_6 = []
        self._syms_4 = []
        for g, (syms, size) in enumerate(TUPLE_GROUPS):
            for sym in syms:
    ...
    def info(self):
        print("N-Tuple Network for Battle 2584:")
        ...
        total_lookups = sum(len(s) for s, _ in TUPLE_GROUPS)
        print(f"  Total: {tw:,} weights, {self.memory_usage_mb():.1f} MB, {total_lookups} lookups/eval")
