...
--------------------------
1. Bootstrap mask uses `terminated` ONLY, never `truncated`. DMC humanoid
   never terminates (`terminated` always 0); episodes only truncate at step
   1000. Conflating the two would teach the critic a finite-horizon problem
   and cripple learning. The buffer hands us both flags as float32 tensors;
   we use `terminated` for the value bootstrap and ignore `truncated` here.
...
class SACAgent:
    ...
    def update(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        Expects `batch` already on `self.device` with keys:
            obs:        (B, obs_dim)    float32
            actions:    (B, action_dim) float32
            rewards:    (B,)            float32
            next_obs:   (B, obs_dim)    float32
            terminated: (B,)            float32  -- bootstrap mask (USE THIS)
            truncated:  (B,)            float32  -- ignored here; do NOT mask with this
        ...
        terminated = batch["terminated"]  # NOTE: not `truncated`. See module docstring.

        # ----- Critic update ----------------------------------------------
        # Target: y = r + gamma * (1 - terminated) * (min Q'(s', a') - alpha * log pi(a'|s'))
        ...
        with torch.no_grad():
            ...
            # Bootstrap mask uses `terminated` only.
