...
DEFAULT_CFG: Dict[str, Any] = {
    ...
    "batch_size": 256,
    # n-step return horizon used by the buffer when constructing TD targets.
    # n_step=1 is byte-identical to the prior 1-step SAC target. n_step=3 is
    # the Q3 lever for humanoid-run -- longer credit chains help when the
    # reward signal is dense but slow to translate into Q-improvement
    # (D4PG uses n=5; we go conservative at n=3 since SAC's actor update
    # already moves more aggressively than DDPG's). Override via --n_step.
    "n_step": 2,

    # --- Loop schedule ------------------------------------------------------
    "warmup_steps": 0,       # random-action steps before any update()
    ...
    "lr_decay_start_frac": 0.5,   # start decay at 60% of total_steps
    "lr_decay_final_factor": 1.0 / 5.0,  # end at 1/3 of initial LR (3e-4 -> 1e-4)
...
def build_buffer(cfg: Dict[str, Any], device: torch.device) -> ReplayBuffer:
    return ReplayBuffer(
        ...
        action_dim=cfg["action_dim"],
        device=device,
        gamma=cfg["gamma"],     # buffer owns the discount math at sample time
        n_step=cfg["n_step"],   # n_step=1 reproduces standard 1-step SAC target
    )
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.n_step is not None:
        assert int(args.n_step) >= 1, f"--n_step must be >= 1, got {args.n_step}"
    ...
    # Cross-check: gamma is now used by both the agent (via cfg, kept for
    # reference / future use) and the buffer (which folds gamma into the
    # n-step `discount` field at sample time). They MUST agree, or the
    # critic's TD target gets a discount factor that doesn't match the
    # gamma the rest of the algorithm assumes. math.isclose to dodge any
    # float coercion subtlety in cfg parsing.
    assert math.isclose(agent.gamma, buffer.gamma), (
        f"gamma mismatch: agent.gamma={agent.gamma}, buffer.gamma={buffer.gamma}"
    )
    ...
    try:
        for step in range(start_step + 1, args.total_steps + 1):
            ...
            if step > cfg["warmup_steps"] and len(buffer) >=cfg["batch_size"]:
...
def parse_args() -> argparse.Namespace:
    ...
    # n-step horizon for the replay buffer's TD target. Default None means
    # "don't override DEFAULT_CFG" (currently 1, i.e. standard 1-step SAC).
    # Pass --n_step 3 to enable 3-step returns for the Q3 run task.
    p.add_argument("--n_step", type=int, default=None,
                   help="n-step return horizon (must be >= 1). "
                        "Default: use DEFAULT_CFG (currently 1 = standard SAC).")
